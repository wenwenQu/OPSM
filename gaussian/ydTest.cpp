...
int main(int argc, char** argv)
{
	...
	if (str == "Apri_ES" || str == "DFS_ES") {
		sprintf(outputfile, "opsm_output_%s_%s_r%d_c%d.txt", dataset, argv[2], minrow, mincol);
		sprintf(outputfileTime, "runtime_opsm_output_%s_%s_r%d_c%d.txt", dataset, argv[2], minrow, mincol);
		sprintf(outputfilePeakMem, "maxmem_opsm_output_%s_%s_r%d_c%d.txt", dataset, argv[2], minrow, mincol);
	}
	else {
		th_prob = atof(argv[5]);
                th_prob_fileName = th_prob * 100;
		sprintf(outputfilePeakMem, "maxmem_opsm_output_%s_%s_r%d_c%d_p%d.txt", dataset,argv[2], minrow, mincol, th_prob_fileName);
		sprintf(outputfile, "opsm_output_%s_%s_r%d_c%d_p%d.txt", dataset,argv[2], minrow, mincol, th_prob_fileName);
		sprintf(outputfileTime, "runtime_opsm_output_%s_%s_r%d_c%d_p%d.txt", dataset,argv[2], minrow, mincol, th_prob_fileName);
	}
