...
struct Range {
	...
	bool contains(double ll, double rr)
	{
		return l <= ll && r >= rr;
	}
...
struct DPMatrix
{
	...
	double** DP;//DP矩阵
	Polynomial<double>* **pr; //多项式矩阵 //todo
	...
	DPMatrix(Range& rng)//DP对应于x=0的base case
	{
		...
		splits = new double[total_std];
		for(int k = 0; k < total_std;k++){
			splits[k] = rng.u +  (k-std_num)* rng.sigma;
		}

		DP = new double*[1];
		DP[0] = new double[2*std_num];
		for(int i = 0; i<2*std_num; i++)
			DP[0][i] = 1;

		pr = new Polynomial<double>** [2 * std_num];
		for (int i = 0; i < 2 * std_num; i++) {
			pr[i] = new Polynomial<double>*[0];
			pr[i][0] = &(seq[0].pdf(i));
		}
	}
	...
	~DPMatrix()
	{
		...
		for(int i = 0; i < rngNum; i++) delete[] pr[i];
